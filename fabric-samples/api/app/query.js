 ...
 * 
     ...
     *queryHospital(ctx, hospitalId)
     *queryPatient(ctx, patientId);

 * 
 */


        if (fcn == "queryAllPatients") {
            //args = parseInt(args);
            result = await contract.evaluateTransaction(fcn);
            result = JSON.parse(result.toString()); 
            console.log(`queryAllPatients data: ${result.toString()}`);
            return result;
        }
        else if(fcn == "queryHospital") {
            //args = parseInt(args);
            result = await contract.evaluateTransaction(fcn, args[0]); 
            result = JSON.parse(result.toString());
            console.log(`queryHospital data:${result.toString()}`);
            return result;
        }
        else if(fcn == "queryPatient") {
            //args = parseInt(args);
            result = await contract.evaluateTransaction(fcn, args[0]); 
            result = JSON.parse(result.toString());
            console.log(`queryPatient data:${result.toString()}`);
            return result;
        }
